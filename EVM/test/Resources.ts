import {
  ...
  describe("Resources", function () {
    ...
    describe("Bonding token", function () {
      ...
      it('User 1 deposits 1000 reserveToken, receives 1000 bondingToken', async function () {
      ...
      it('User 2 deposits 1000 reserveToken, receives 1000 bondingToken', async function () {
        const { bondingToken, user1 } = await loadFixture(deploy);
        const depositAmt = hre.ethers.parseEther("1000");
        await bondingToken.connect(user1).deposit(depositAmt, user1);
        const bal = await bondingToken.balanceOf(user1);
        expect(bal).to.equal(depositAmt);
      });
      it('User 1 redeems 1000 bondingToken, receives 1000 reserveToken', async function () {
        const { bondingToken, user } = await loadFixture(deploy);
      ...
      it('User 2 redeems 1000 bondingToken, receives 1000 reserveToken', async function () {
        const { bondingToken, user1 } = await loadFixture(deploy);

        const depositAmt = hre.ethers.parseEther("1000");
        await bondingToken.connect(user1).deposit(depositAmt, user1);
        const depBal = await bondingToken.balanceOf(user1);
        expect(depBal).to.equal(depositAmt);
