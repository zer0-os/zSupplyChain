  ...
  for (const contract of contractNames) {
    describe(`${contract.name} simulation tests`, function () {
      async function deploy() {
        ...
        const bondingTokenFactory = await hre.ethers.getContractFactory("BondingTokenLinear");
        const bondingToken = await bondingTokenFactory.deploy("GOLD", "GOLD", reserveTokenAddress, 0, 0);
      ...
      async function getExpectedShares(bondingToken: BondingTokenLinear, assets: bigint) {
        return await bondingToken.previewDeposit(assets);
      }

      async function getExpectedAssets(bondingToken: BondingTokenLinear, shares: bigint) {
              ...
              it(`should simulate econ with random deposits and redeems for ${userCount} users with entry fee ${entryFee} bps and exit fee ${exitFee} bps`, async function () {
                ...
                for (const user of selectedUsers) {
                  ...
                  for (let i = 0; i < 3; i++) { 
                    ...
                    if(amount > max ){
                      await expect(getExpectedShares(bondingToken, amount)).to.be.revertedWith("ERC4626ExceededMaxDeposit");
                      continue;
                    }else{
                      expectedShares = await getExpectedShares(bondingToken, amount);
                    }
      ...
      describe('Verify edge cases', function () {
        ...
        entryFees.forEach(entryFee => {
          exitFees.forEach(exitFee => {
            ...
            const deposits = [10n, 10n**2n, 10n**3n, 10n**8n, 10n**18n, 10n**22n, 10n**32n];
            numUsers.forEach(userCount => {
              ...
              it(`should check edge cases with exact deposits and redeems for ${userCount} users with entry fee ${entryFee} bps and exit fee ${exitFee} bps`, async function () {
                ...
                for (const user of selectedUsers) {
                  ...
                  for(const deposit of deposits){
                      ...
                      if(deposit > max ){
                        await expect(bondingToken.connect(user).deposit(amount, userAddress)).to.be.revertedWithCustomError(bondingToken, "ERC4626ExceededMaxDeposit");
                        continue;
                      }else{
                        expectedShares = await getExpectedShares(bondingToken, amount);
                      }
